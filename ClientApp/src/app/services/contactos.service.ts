...
export class ContactosService {
 myAppUrl = 'https://localhost:44390/api/contactos/';
  ...
  deleteContacto(Id_usuario:number): Observable<Contacto[]>{
    return this.http.delete<Contacto[]>(this.myAppUrl + Id_usuario);
  }
  guardarContcato(contacto: Contacto): Observable<Contacto> {
    return this.http.post<Contacto>(this.myAppUrl , contacto, this.httpOptions);
  }
  cargarContacto(Id_usuario:number): Observable<Contacto> {
    return this.http.get<Contacto>(this.myAppUrl + Id_usuario);
  }
  actualizarContacto(Id_usuario:number,contacto:Contacto): Observable <Contacto>{
    return this.http.put<Contacto>(this.myAppUrl  + Id_usuario, contacto, this.httpOptions);
  }
}
